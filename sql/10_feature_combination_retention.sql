...
SELECT
    CASE
        WHEN m.target_rate_user_flag = 1
         AND m.rate_alert_user_flag = 1
            THEN 'Both target-rate and rate-alert'

        WHEN m.target_rate_user_flag = 1
         AND m.rate_alert_user_flag = 0
            THEN 'Target-rate only'

        WHEN m.target_rate_user_flag = 0
         AND m.rate_alert_user_flag = 1
            THEN 'Rate-alert only'
