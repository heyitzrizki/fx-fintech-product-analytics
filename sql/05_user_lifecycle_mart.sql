...
SELECT
    ...
    ROUND(
        julianday(u.kyc_completed_at)
        - julianday(u.signup_timestamp),
        2
    ) AS days_to_kyc,

    ROUND(
        julianday(st.first_successful_transaction_at)
        - julianday(u.signup_timestamp),
        2
    ) AS activation_days_exact,
    ...
    COALESCE(st.successful_transaction_count, 0)
        AS successful_transaction_count,

    COALESCE(st.successful_transaction_volume, 0)
        AS successful_transaction_volume,

    COALESCE(st.target_rate_user_flag, 0)
        AS target_rate_user_flag,

    COALESCE(ra.rate_alert_user_flag, 0)
        AS rate_alert_user_flag,

    CASE
        WHEN st.first_successful_transaction_at IS NOT NULL
         AND u.kyc_completed_at IS NULL
        THEN 0
        ELSE 1
    END AS onboarding_valid_flag,
    ...
    CASE
        WHEN u.kyc_completed_at IS NOT NULL
         ...
         AND datetime(st.first_successful_transaction_at)
             <= datetime(u.signup_timestamp, '+14 days')
        THEN 1
        ELSE 0
...
LEFT JOIN successful_transactions st
    ON u.user_id = st.user_id

LEFT JOIN rate_alert_users ra
